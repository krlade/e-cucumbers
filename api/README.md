...
| `GET` | `/api/nodes/telemetry/?device_id=2137` | ✅ Bearer JWT (user lub device) | Pobiera historię odczytów telemetrii dla gateway'a. Opcjonalne filtrowanie po typie czujnika i limicie. Wyniki zwracane w porządku rosnącym (`recorded_at` ASC) — gotowe do wykresów. | `?device_id=2137&sensor_type=temperature&limit=100` | `200` — lista `TelemetryReading` |
